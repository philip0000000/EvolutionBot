...
void MLPBox::RandomiseMLPBox()
{
	// make all w, id & type to 0
	for (int m = 0; m < CONNS; m++)
		w[m] = 0,
		id[m] = 0,
		type[m] = 0;
...
MLPBrain::MLPBrain()
{
	boxes.resize(BRAINSIZE);
}
void MLPBrain::RandomiseMLPBrain()
{
	for (int m = 0; m < BRAINSIZE; m++)
		boxes[m].RandomiseMLPBox();
}
...
void MLPBrain::tick(MyVector<float>& in, MyVector<float>& out)
{
...
void MLPBrain::liveMutate(float MR, float MR2, MyVector<float>& out)
{
